...
| 3 | **Pricing section viewers, 90 days** | Custom > Website > Events > `ViewContent` where `content_name` equals `pricing` > 90 days | Fed by the PostHog to pixel mapping in `pixel-install.md`. Highest intent pool, and it will also be the smallest. Live since 2026-09-14 (decision P-6) and dwell-gated at 50% for 2s, so it collects roughly four weeks behind audience 2. Do not expect a usable size before mid-October. |
