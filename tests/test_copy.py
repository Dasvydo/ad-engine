...
def test_evidence_records_the_dated_decisions():
    ...
    # Was `all(... == "2026-09-06")`, which its own branch broke on 2026-09-14
    # when a supersession was appended. A decision log that cannot record a later
    # decision is not a log. Both founding decisions keep their date; anything
    # superseding one must be dated after it and say so.
    by_topic = {d["topic"]: d for d in ev["_decisions"]}
    assert by_topic["ROI figures"]["date"] == "2026-09-06"
    assert by_topic["Price and destination"]["date"] == "2026-09-06"
    for d in ev["_decisions"]:
