...
| --- | --- | --- | --- | --- |
| **Meta Ad Library API** (`ads_archive`) | **~200 calls per hour per token.** Reported by every secondary guide; Meta's reference names the error code (613) and not the number. Free - there is no paid tier to overspend into | **20 to 40 calls a sweep** with `research/seeds.yaml` as it ships: 15 seeded pages and 5 queries, ONE SEARCH EACH, paginated to at most `DEFAULT_MAX_PAGES = 2`. Pages used to go ten to a `search_page_ids` call, which is what the API accepts and cost 6 to 12 - but MEASURED 2026-09-19 that batching returned 50 Fyxer ads and nothing of Jace.ai's from a two-page call, because the archive orders a result set by recency across the whole batch and the busier page takes every slot. The extra calls buy ads that were previously asked for and silently not returned; see `discover.PAGE_IDS_PER_SEARCH`. `research.yml` passes `--budget 150`, not the module default of 200, so a sweep cannot eat a whole hour's allowance and leave nothing for a re-run. Measure adds 0 to this row - it is a different endpoint on the same token | **Arithmetic**, on `engine/discover.py`'s own `plan()`. `python -m engine.fanout --dry-run` prints the spread for the seed file as it stands, with no token and no socket, and `tests/test_fanout.py` re-derives `searches` and `ad_library_calls` from the committed seeds rather than pinning a figure, and asserts the headroom: three sweeps inside `--budget 150`. Never called live | Error 613, never a charge. `Quota(150)` is charged **before** the socket opens and refuses the call it cannot afford, carrying `needed`, `remaining` and `budget` in the message. Seeding a page that runs no ads in DK or LT is the way this row grows for nothing: a page that returns nothing costs a call every week |
...
06:00  research.yml   20-40 Ad Library calls, up to 4 model calls, ~2m
